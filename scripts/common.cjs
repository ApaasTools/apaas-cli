...
function loadApaasConfig(customModule, apaasJson = 'apaas.json') {
  const maybePath = typeof customModule === 'string' ? customModule.trim() : '';

  // 支持两种模式：
  // 1) 传模块目录路径（相对/绝对）：例如 ./apps/apaas-custom-editor
  // 2) 传模块名：例如 apaas-custom-editor（会自动拼 apps/<name>）
  let customModulePath;
  if (maybePath && (maybePath.startsWith('.') || maybePath.startsWith('/') || maybePath.includes(path.sep))) {
    customModulePath = path.isAbsolute(maybePath)
      ? maybePath
      : resolvePath(maybePath);
  } else {
    customModulePath = resolvePath('apps', customModule);
  }

  const configPath = resolvePath(customModulePath, apaasJson);

  if (!fs.existsSync(customModulePath)) {
    exitWithError(`模块目录不存在：${customModulePath}`);
  }

  if (!fs.existsSync(configPath)) {
    exitWithError(`模块目录 ${customModulePath} 下不存在需要的 ${apaasJson} 文件`);
  }
