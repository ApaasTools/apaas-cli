...
const lib = async (name, cmd, program) => {
  if (!name) {
    console.log(`未指定插件名`['red'])
    process.exit(0)
  }
  const customModule = 'apaas-custom-plugin-' + name
  ...
  writeApaasJson(apaasConfig.code, apaasConfig)
  zip(apaasConfig.code, apaasConfig.code)
}
...
const doLib = (apaasModuleConfig, customModulePath) => {
  ...
  const outputName = apaasModuleConfig.code + '/' + moduleType
